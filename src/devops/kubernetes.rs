...
fn check_resource_limits(source: &str, diagnostics: &mut Vec<Diagnostic>) {
    if source.contains("containers:") && !source.contains("resources:") {
        diagnostics.push(Diagnostic {
            rule: "k8s-manifest",
            message: "containers without resource limits — add resources.requests and resources.limits".to_string(),
            line: 1,
    ...
    for (i, line) in source.lines().enumerate() {
        ...
        if trimmed.starts_with("image:") || trimmed.starts_with("- image:") {
            ...
            if image == "latest" || (!trimmed.contains(':') && trimmed.contains("image:")) {
                ...
                if img_val.ends_with(":latest") || (!img_val.contains(':') && !img_val.contains('@') && !img_val.is_empty()) {
                    diagnostics.push(Diagnostic {
                        rule: "k8s-manifest",
...
fn check_wildcard_rbac(source: &str, diagnostics: &mut Vec<Diagnostic>) {
    for (i, line) in source.lines().enumerate() {
        let trimmed = line.trim();
        if (trimmed.contains("resources:") || trimmed.contains("verbs:")) && trimmed.contains("\"*\"") {
            diagnostics.push(Diagnostic {
                rule: "k8s-manifest",
